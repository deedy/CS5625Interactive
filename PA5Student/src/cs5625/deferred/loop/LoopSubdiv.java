...
public class LoopSubdiv {
	...
	public LoopSubdiv(EdgeDS edgeDS) {
		...
		for (Integer edgeID : edgeDS.getEdgeIDs()) {
			...
			newEdges.put(edge0n, new EdgeData(vert0, newVertex)); // For each
																	// edge put
																	// an edge
																	// between
																	// the one
																	...
																	// and the
																	// new one
			newEdges.put(edgen1, new EdgeData(newVertex, vert1)); // and an edge
																	// between
																	// the new
																	// one and
																	// the other
		...
		for (Integer polyID : edgeDS.getPolygonIDs()) {
			...
			newPolys.put(newPolyID++, new PolygonData(e1, edgenv2nv0, e6,
					vert0, newvert0, newvert2));
			newPolys.put(newPolyID++, new PolygonData(e2, e3, edgenv0nv1,
					newvert0, vert1, newvert1));
			newPolys.put(newPolyID++, new PolygonData(edgenv1nv2, e4, e5,
					newvert2, newvert1, vert2));
			newPolys.put(newPolyID++, new PolygonData(edgenv0nv1, edgenv1nv2,
					edgenv2nv0, newvert0, newvert1, newvert2));
